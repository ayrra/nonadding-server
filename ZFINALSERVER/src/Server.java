...
public class Server {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		double[] numbers = new double[1000];  //we will use 1001 since we will be starting at 1
		double[] tempSend = new double[10];   //this array we use for sending out the 10's of the array
		int servPort = 444;	//port number the server will run on
		int count = 1;		//start at 1 so the code triggers at numOfClients + 1
		int numOfClients = 5;	//CHANGE THIS TO HOWEVER MANY CLIENTS YOU WANT TO USE
		boolean isLoopDone = false;		//dont change, we use this when we are completely finished
		int frames = 0;		//frames will keep track of which frame we're on    
		int low = 0;		//low and high used for keeping track of array bounds
		int high = 9;		
		...
		while (true) {		
			...
			if (count > numOfClients) {
				for (int i = 0; i < 100; i++) {	//this will run 100 times so 100 x 10 elements = 1000
					...
					if (frames > numOfClients-1)		//when we reach 5 frames we reset back to 0
						frames = 0;
				}
				...
				for (int i = 0; i < 10; i++) {		//we repeat to do the 100 elements
			...
			if (isLoopDone == true) { //answer is printed and we send stop to all the clients
				...
				double totalSum = Arrays.stream(numbers).sum();
				System.out.println("Local server sum (just to verify) is: " + totalSum);
				sendStop(sockets);
